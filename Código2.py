...
def tournament_selection(population, distance_matrix, k=5):
    ...
    return min(selected, key=lambda route: calculate_route_distance(route, distance_matrix))
...
def mutate(child, mutation_rate):
    if random.random() < mutation_rate:
        ...
        child[start:end] = reversed(child[start:end])  # Inversión
    return child


def TSP_genetic_algorithm(cities, population_size=200, generations=1000, mutation_rate=0.01):
    ...
    for generation in range(generations):
        ...
        next_generation = population[: population_size // 4]  # Mantiene el 25% de los mejores

        # Generar nuevos individuos con cruce y mutación
